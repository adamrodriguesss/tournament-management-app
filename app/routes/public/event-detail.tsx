...
import { getEventRegistrations } from '../../services/bracket';
...
export async function clientLoader({ params }: Route.ClientLoaderArgs) {
  ...
  if (!event) {
    return { event: null, matches: [], registrations: [], results: [], isLoggedIn: !!session, tournamentId };
  }
  ...
  if (event.format === 'bracket' && matches.length === 0) {
    const regsRes = await getEventRegistrations(eventId);
...
export default function EventDetail({ loaderData }: { loaderData: any }) {
  ...
  const { event, matches, registrations, results, isLoggedIn, tournamentId } = loaderData;
  ...
  return (
    <div className="min-h-screen bg-pixel-black text-pixel-slate-light flex flex-col">
      ...
      <main className="flex-1 p-4 sm:p-6 md:p-8 max-w-5xl mx-auto w-full">
              ...
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {registrations.map((reg: any) => {
                    const name = reg.team?.name || reg.participant?.user?.full_name || 'Unknown';
                    const dept = reg.team?.department || reg.participant?.team?.department || 'N/A';
                    return (
                      <div
                        key={reg.id}
                        className="bg-pixel-card border-[3px] border-pixel-border p-4 flex flex-col gap-1"
                        style={{ boxShadow: '3px 3px 0 var(--color-pixel-border)' }}
                      >
                        <span className="font-[family-name:var(--font-pixel)] text-[11px] text-pixel-gold truncate">
                          {name.toUpperCase()}
                        </span>
                        <span className="font-[family-name:var(--font-vt)] text-[18px] text-pixel-slate">
                          {dept}
                        </span>
                      </div>
                    );
                ...
                <div className="mt-6 border-2 border-pixel-cyan-dim bg-pixel-cyan/5 p-4 font-[family-name:var(--font-vt)] text-[18px] text-pixel-slate text-center">
                  The tournament bracket will be generated once registrations are finalized.
                </div>
              </>
            ) : null}
          </div>
        ...
        {/* Empty State */}
        {results.length === 0 && matches.length === 0 && registrations.length === 0 && (
          <div
          ...
          >
            ...
            <p className="text-xl font-[family-name:var(--font-vt)] text-pixel-slate">
              No registrations or matches are available for this event yet.
            </p>
          </div>
        )}
      </main>
